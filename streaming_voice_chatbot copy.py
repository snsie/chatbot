...
SYSTEM_PROMPT = "You are a concise, helpful voice assistant. Keep answers short unless asked for detail."
MAX_TOKENS = 512
...
class CoquiTTSSpeaker(BaseSpeaker):
    ...
    def __init__(self, model_name: Optional[str] = VOICE_NAME):
        ...
        # Initialize TTS model with CPU to avoid conflicts
        self.device = "cpu"  # Use CPU to avoid CUDA/audio device conflicts
        print(f"[CoquiTTS] Loading model {self.model_name} on {self.device}...")
        
        try:
            self.tts = TTS(model_name=self.model_name).to(self.device)
        except Exception as e:
            print(f"[CoquiTTS] Failed to load {self.model_name}, falling back to simpler model...")
    ...
    async def speak(self, sentence: str):
        ...
        try:
            ...
            if len(wav) > 0:
                ...
                if max_val > 0:
                    wav = wav / max_val * 0.8  # Leave some headroom
            
            # Use sounddevice for more reliable playback
            sample_rate = getattr(self.tts.synthesizer.output_sample_rate, 'value', 22050) if hasattr(self.tts, 'synthesizer') else 22050
            
            # Play using sounddevice (more reliable than simpleaudio)
