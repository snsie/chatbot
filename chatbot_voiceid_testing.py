...
### for crowds
CROWD_MODE = True          # set False to instantly revert to your old behavior
USE_SEPARATION = CROWD_MODE
SIM_THRESHOLD = 0.65 if CROWD_MODE else 0.55
...
async def process_turn(detector: UtteranceDetector, stt: WhisperSTT, convo: Conversation, speaker: BaseSpeaker):
    ...
    clean_audio, best_sim, best_idx, used_sep = extract_target_voice(
        audio,
        16000,
        target_emb,
        sep_model=sep_model,
        spkrec_model=spkrec,
        max_speakers=2,
        sim_floor=SIM_THRESHOLD,
        enable=should_try_sep,
        device=device,
    )

    if used_sep:
        if best_idx >= 0:
            print(f"[SEP] chose source {best_idx} (cos={best_sim:.2f})")
        else:
